...
class _TransactionSummaryWidgetState extends State<TransactionSummaryWidget> {
    ...
    try {
      ...
      for (var transaction in response) {
        ...
        final type = transaction['type'] ??
            'Not Specified'; // Default to empty string if null
        final category =
            transaction['category'] ?? 'others'; // Default to 'others' if null
  ...
  Future<void> _generateAndUploadPdf() async {
    try {
                    ...
                    child: pw.Chart(
                      grid: pw.CartesianGrid(
                        ...
                        yAxis: pw.FixedAxis(
                          [
                            0,
                            effectiveMaxValue * 0.25,
                            effectiveMaxValue * 0.5,
                            effectiveMaxValue * 0.75,
                            effectiveMaxValue
                          ],
                        ...
                        (entry) => pw.TableRow(
                          children: [
                            ...
                            pw.Padding(
                              padding: const pw.EdgeInsets.all(5),
                              child: pw.Text(
                                  'BDT.${entry.value.toStringAsFixed(2)}'),
                            ),
                            pw.Padding(
                              padding: const pw.EdgeInsets.all(5),
                              child: pw.Text(
      ...
      final filename =
          'transaction_summary_${widget.eventId}_${DateTime.now().millisecondsSinceEpoch}.pdf';

      await _supabase.storage.from('PDF').uploadBinary(filename, pdfBytes);

      final fileUrl =
          await _supabase.storage.from('PDF').getPublicUrl(filename);
